...
with open('../../../ref_pompejana/Apomp_vs_REF_oldgenes_short.csv','r') as fin:
...
pep_seqs = {}
for root, dirs, files in os.walk("../../../proteomes/oldgenes/", topdown=False):
	for name in files:
		if 'PEP' in name:
			sp = name.split('_')[0]
			pep_seqs[sp] = {}
			with open('../../../proteomes/oldgenes/'+name,'r') as fin:
				for l in fin.readlines():
					if l[0] == '>':
						n = l[1:].strip('\n')
					else:
...
for k in dicseq:
	...
	if pando == 1 and uni == 1 and para1+para2 >=1: #condition minimale
		...
		if k in blast_apomp:
			...
			with open('../../../gene_alignments/oldgenes/'+'-'.join(conv_apomp[blast_apomp[k]])+'_'+phylo+'.fa','w') as fout: #nom pour l'alignement
